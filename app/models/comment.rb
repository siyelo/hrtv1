class Comment < ActiveRecord::Base
  ...
  scope :by_projects, :joins => "JOIN comments c ON c.commentable_id = projects.id "
  scope :on_projects_for, lambda { |organization|
          { :joins => "JOIN projects p ON p.id = comments.commentable_id ",
            :conditions => ["p.data_response_id IN (?)", organization.data_responses.map(&:id) ]
          }
        }
  scope :on_funding_sources_for, lambda { |organization|
  ...
  scope :on_implementers_for, lambda { |organization|
          { :joins => "JOIN funding_flows f ON f.id = comments.commentable_id ",
  ...
  scope :on_activities_for, lambda { |organization|
