...
module MapFields
  ...
  def map_fields
    ...
    if session[:map_fields].nil? || !params[options[:file_field]].blank?
      ...
      #TODO don't save file to filesystem
      # that will randomly not work
      #  see http://docs.heroku.com/constraints#read-only-filesystem
      # instead save to database
      # and add appropriate keys to map_session
      # or parse and save to a database temp table
